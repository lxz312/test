<template>
  <div class="app-container">
    <div class="filter-container">
      <el-form :inline="true">
        <el-form-item>
          <el-button type="success" icon="el-icon-back" @click.native.prevent="$router.back(-1)">返回</el-button>
          <el-button
            type="primary"
            icon="el-icon-plus"
    ...
    >
      ...
      >
        <el-form-item label="专业大类编码" prop="major_id" required>
          <el-input type="text" auto-complete="off" v-model="tmpData.major_id" :disabled="true" />
        </el-form-item>
        ...
        <el-form-item label="第N主考院校" prop="main_target_school_code">
          <el-input type="number" auto-complete="off" v-model="tmpData.main_target_school_code" />
        </el-form-item>
      </el-form>
...
import {
  schoolList as list,
  schoolSearch as search,
  schoolRemove as remove,
  schoolAdd as add,
  schoolUpdate as update,
  schoolUpdateByBatch as updateByBatch,
} from "@/api/major";
...
export default {
  ...
  methods: {
    ...
    getDataList() {
      ...
      list(this.listQuery, this.majorData.major_id)
        .then((response) => {
    ...
    searchData() {
      ...
      search(this.search, this.majorData.major_id)
        .then((response) => {
          this.dataList = response.data.list;
          this.total = response.data.total;
          this.listLoading = false;
