<template>
  <div class="app-container">
    ...
    >
      <el-table-column v-if="operationStatus === 'batch'" type="selection" width="40"></el-table-column>
      <el-table-column label="#" align="center" width="80">
        <template slot-scope="scope">
          <span v-text="getIndex(scope.$index)"></span>
        </template>
      </el-table-column>
      <el-table-column label="专业编码" align="center" prop="major_id" width="120" />
      <el-table-column label="大类专业编码" align="center" prop="major_category_code" width="120" />
      <el-table-column label="国家专业编码" align="center" prop="national_major_code" width="120" />
      <el-table-column label="专业名称" align="center" prop="major_name" width="120" />
      <el-table-column sortable prop="major_status" label="专业状态" width="100" align="center">
        <template slot-scope="scope">
          <el-tag
            :type="scope.row.major_status === '正常' ? 'success'
            : scope.row.major_status === '即将停考' ? 'warning' : 'danger'"
            disable-transitions
          >{{scope.row.major_status}}</el-tag>
        </template>
      </el-table-column>
      ...
      >
        <template slot-scope="scope">
          <router-link
            ...
            actionStatus:'update'  } }"
          >
            <el-button type="primary" size="mini" v-if="hasPermission('role:detail')">修改</el-button>
          </router-link>
          <router-link
            class="inlineBlock"
            ...
            actionStatus:'add'  } }"
          >
            <el-button type="primary" size="mini" v-if="hasPermission('role:list')">主考院校</el-button>
          </router-link>
          <el-button
            type="danger"
            size="mini"
    ...
    <el-dialog :visible.sync="dialogFormVisible">
      <el-form
        ...
        ref="tmpData"
      >
        <el-form-item prop="national_major_code" label="国家专业代码">
          <el-input v-model="tmpData.national_major_code" placeholder="7 位编码（1 位字符+6 位数字）" />
        </el-form-item>
        ...
        <el-form-item label="学历层次">
          <el-select
            v-model="tmpData.education_level"
            placeholder="请选择学历层次"
            :value="tmpData.education_level"
          >
            <el-option label="专科" value="专科"></el-option>
        ...
        <el-form-item label="首次开考考次" prop="first_exam_id">
          <el-input v-model="tmpData.first_exam_id" placeholder="3 位数字编码（例：171-17 年 4 月考试）" />
        </el-form-item>
        <el-form-item label="专业状态">
          <el-select
            v-model="tmpData.major_status"
            placeholder="请选择专业状态"
            :value="tmpData.major_status"
          >
        ...
        <el-form-item label="总学分">
          <el-input v-model="tmpData.total_credit" type="number" />
        </el-form-item>
        <el-form-item label="毕业学分">
          <el-input v-model="tmpData.graduation_credit" type="number" />
        </el-form-item>
        <el-form-item label="总课程数">
          <el-input v-model="tmpData.total_course_number" type="number" />
        </el-form-item>
        <el-form-item label="是否分方向">
          <el-col :span="12">
        ...
        <el-form-item prop="major_category_code" label="专业大类代码">
          <el-input v-model="tmpData.major_category_code" />
        </el-form-item>
        <el-form-item label="报考条件说明">
          <el-input type="textarea" autosize placeholder="请输入内容" v-model="tmpData.apply_condition"></el-input>
        </el-form-item>
        ...
        <el-form-item label="批准文号">
          <el-input v-model="tmpData.approve_num" />
        </el-form-item>
        <el-form-item label="停止新生注册考次" prop="stop_freshman_registration_exam_id">
          <el-input
            v-model="tmpData.stop_freshman_registration_exam_id"
            placeholder="3 位数字编码（例：171-17 年 4 月考试）"
          />
        </el-form-item>
        <el-form-item label="停止注册文号">
          <el-input v-model="tmpData.stop_registration_num" />
        </el-form-item>
        <el-form-item label="停止报考考次" prop="stop_apply_exam_id">
          <el-input v-model="tmpData.stop_apply_exam_id" placeholder="3 位数字编码（例：171-17 年 4 月考试）" />
        </el-form-item>
        <el-form-item label="停止报考文号">
          <el-input v-model="tmpData.stop_apply_num" />
        </el-form-item>
        <el-form-item label="停止颁发毕业证日期">
          <el-date-picker v-model="tmpData.stop_diploma_date" type="date" placeholder="停止颁发毕业证日期"></el-date-picker>
        </el-form-item>
        <el-form-item label="停止颁发文号">
          <el-input v-model="tmpData.stop_diploma_num" />
        </el-form-item>
        <el-form-item label="备注">
          <el-input type="textarea" autosize placeholder="请输入内容" v-model="tmpData.notes"></el-input>
        </el-form-item>
      </el-form>
        ...
        first_exam_id: [
          {
            min: 3,
            max: 3,
            message: "3 位数字编码（例：171-17 年 4 月考试）",
            trigger: "blur",
          },
        ],
        stop_freshman_registration_exam_id: [
  ...
  methods: {
    ...
    updateDataByBatch() {
      ...
      this.$refs.tmpData.validate((valid) => {
        if (valid) {
          updateByBatch(this.tmpData, ids)
            .then(() => {
