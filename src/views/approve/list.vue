<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="申请表编码" align="center" prop="approve_id" width="120" />
      <el-table-column label="申请表名称" align="center" prop="approve_name" width="120" />
      <el-table-column label="申请院校" align="center" prop="school_name" width="120" />
      <el-table-column sortable prop="approve_status" label="申请表状态" width="100" align="center">
        <template slot-scope="scope">
          <el-tag
            :type="scope.row.approve_status === '已审批' ? 'success'
            : scope.row.approve_status === '已申请' ? 'warning' : 'danger'"
            disable-transitions
          >{{scope.row.approve_status}}</el-tag>
        </template>
      </el-table-column>
      <el-table-column label="创建时间" align="center" prop="apply_time" width="240" />
      <el-table-column
        label="管理"
        align="center"
        ...
        <template slot-scope="scope">
          ...
          >
            <el-button
              type="primary"
              size="mini"
              :disabled="hasPermission('role:update') &&  scope.row.approve_status == '已申请'"
            >审核</el-button>
          </router-link>
          <el-button
            type="danger"
    ...
    >
      ...
      >
        ...
        <el-form-item label="院校名称" prop="school_name" required>
          <el-input type="text" auto-complete="off" v-model="tmpData.school_name" :disabled="true" />
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        ...
        >
          <el-button type="primary" icon="el-icon-plus" v-if="hasPermission('role:add')">新专业</el-button>
        </router-link>
        <router-link
          class="inlineBlock"
          :to="{ path:'/approve/selectmajor_list/', query:{school_id:this.tmpData.school_id, ShowStatus:'adjust'} }"
        >
          <el-button type="primary" icon="el-icon-plus" v-if="hasPermission('role:add')">专业调整</el-button>
        </router-link>
        <router-link
          class="inlineBlock"
          :to="{ path:'/approve/selectmajor_list/', query:{school_id:this.tmpData.school_id, ShowStatus:'continue'} }"
        >
          <el-button type="primary" icon="el-icon-plus" v-if="hasPermission('role:add')">续办专业</el-button>
        </router-link>
        <router-link
          class="inlineBlock"
          :to="{ path:'/approve/selectmajor_list/', query:{school_id:this.tmpData.school_id, ShowStatus:'extend'} }"
        >
          <el-button type="primary" icon="el-icon-plus" v-if="hasPermission('role:add')">扩办专业</el-button>
        </router-link>
      </div>
    </el-dialog>
    <el-dialog :visible.sync="dialogFormVisible" v-if="operationStatus === 'school'">
      <el-table :data="schoolDataList" highlight-current-row append-to-body>
...
export default {
  ...
  methods: {
    ...
    showAddDialog() {
      ...
      this.tmpData.school_id = "10001";
      this.tmpData.school_name = "北京大学";
      this.operationStatus = "single";
    },
    getSchoolList() {
      this.operationStatus = "school";
      var me = this;
      schoolList(this.listQuery)
        .then((response) => {
