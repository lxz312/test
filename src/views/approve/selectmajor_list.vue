    ...
    <div class="filter-container">
      <el-form :inline="true">
        <el-form-item>
          ...
          >批量添加</el-button>
          <el-button v-if="hasPermission('role:list')" @click="$router.back(-1)">返回</el-button>
        </el-form-item>
      </el-form>
    </div>
    <el-table
  ...
  methods: {
    ...
    getDataList(ShowStatus) {
      if (this.ShowStatus === "continue" || this.ShowStatus === "adjust") {
        ...
        continuelist(this.listQuery, this.tmpData.school_id)
          .then((response) => {
        ...
        console.log("this.tmpData.school_id", this.tmpData.school_id);
        extendlist(this.listQuery, this.tmpData.school_id)
          .then((response) => {
            this.dataList = response.data.list;
            this.total = response.data.total;
            this.listLoading = false;
    ...
    addDataByBatch() {
      ...
      addApproveMajorByBatch(param, ids)
        .then(() => {
